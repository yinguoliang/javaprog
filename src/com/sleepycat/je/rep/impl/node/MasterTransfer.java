...
public class MasterTransfer {
     ...
     * discovers that it is no longer master (the {@link
    ...
    private String chooseReplica() throws InterruptedException {
        ...
        String result = null;
        for (;;) {
            final long timeout = deadlineTimeMs - System.currentTimeMillis();
            final VLSNProgress event =
                queue.pollOrException(timeout, TimeUnit.MILLISECONDS);
            if (event == null) {
                return null;
            }
